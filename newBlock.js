...
function pugConnect(blockName) {
    return new Promise((resolve, reject) => {
        fs.appendFile(`src/templates/_layout/_includes/blocks.pug`, `//- block ${blockName} \ninclude ../../../blocks/${blockName}/${blockName}.pug \n\n`, err => {
            if (err) {
...
function styleConnect(blockName) {
    return new Promise((resolve, reject) => {
        fs.appendFile(`src/blocks/_blocks.css`, `/* block ${blockName} */ \n@import "${blockName}/${blockName}.css";\n\n`, err => {
            if (err) {
                reject(`ERR>>> Failed to rewrite blocks.pug`);
            } else {
                resolve();
    ...
    const makeBlock = blockName => {
        ...
        return validateBlockName(blockName)
            ...
            .then(files => {
                ...
                console.log(line);
                console.log('');
                console.log(`Компонент ${blockName} создан`);
                console.log(line);
